    ...
    public List<Leave> getEmployeeLeavesByManagerId(int userId) {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                leave.setAppliedBy(resultSet.getInt("applied_by"));
                leave.setEmployeeName(resultSet.getString("name"));
                leave.setDateFrom(resultSet.getDate("date_from").toLocalDate());
                leave.setDateTo(resultSet.getDate("date_to").toLocalDate());
                leave.setLeaveId(resultSet.getInt("leave_id"));
                leave.setLeaveName(LeaveName.valueOf(resultSet.getString("leave_name")));
                leave.setStatus(LeaveStatus.valueOf(resultSet.getString("status")));
                leave.setReason(resultSet.getString("reason"));
                leave.setCreatedAt(LocalDate.from(resultSet.getTimestamp("created_at").toLocalDateTime()));
                leave.setAppliedDate(resultSet.getTimestamp("applied_date").toLocalDateTime().toLocalDate());
